...
#ifndef PACKEDARRAY_IMPL_FALLTHROUGH
#if defined(__has_attribute)
#if __has_attribute(fallthrough)
#define PACKEDARRAY_IMPL_FALLTHROUGH __attribute__((fallthrough))
#endif
#endif
#ifndef PACKEDARRAY_IMPL_FALLTHROUGH
...
#define PACKEDARRAY_IMPL_FALLTHROUGH __attribute__((fallthrough))
#else
#define PACKEDARRAY_IMPL_FALLTHROUGH ((void)0)
#endif
#endif
#endif
...
#if defined(PACKEDARRAY_IMPL_LOOP_CASES) && PACKEDARRAY_IMPL_CASE_I == 0
        PACKEDARRAY_IMPL_FALLTHROUGH;
#endif
...
void PACKEDARRAY_JOIN(__PackedArray_pack_, PACKEDARRAY_IMPL_BITS_PER_ITEM)(uint32_t* __restrict out, uint32_t offset, const uint32_t* __restrict in, uint32_t count)
{
  ...
  if (count >= 32 - offset)
  {
    ...
    switch (offset)
    {
      do
      {
#define PACKEDARRAY_IMPL_LOOP_CASES
#define PACKEDARRAY_IMPL_PACK_CASES
#include PACKEDARRAY_SELF
#undef PACKEDARRAY_IMPL_LOOP_CASES
      } while (--n > 0);
    }
