...
// При выполнении задания будьте аккуратны 
// при работе с памятью — при вызове метода не должно возникать утечек памяти. 
...
// В шаблоне вы увидите объявления уже известных вам конструкторов и деструкторов, 
// реализовывать их заново не нужно, они уже реализованы. 
...
struct String {
	...
	void append(String &other) {
        size_t otherSize = strlen(other.str);
        char * tempStrPtr = new char [size+otherSize+1];
        strcpy(tempStrPtr, str);
        strcpy(tempStrPtr+size, other.str);
        
        delete [] str;
        str = new char [size+otherSize+1];
